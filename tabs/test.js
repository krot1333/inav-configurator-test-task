...
TABS.test.initialize = function (callback) {
    ...
    function load_html() {
        GUI.load(path.join(__dirname, "test.html"), Settings.processHtml(process_html));
    }
...
TABS.test.cleanup = function (callback) {
    if (callback) {
        callback();
    }
};
